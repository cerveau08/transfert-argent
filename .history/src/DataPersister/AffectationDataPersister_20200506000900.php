...
class AffectationDataPersister implements ContextAwareDataPersisterInterface
{
    ...
    public function __construct(EntityManagerInterface $entityManager, TokenStorageInterface $tokenStorage,UserPasswordEncoderInterface $userPasswordEncoder, AffectationRepository $affectCompteRepo)
    {
              ...
              //  {
                        ...
                        $data->getUserComptePartenaire()->getRoles()[0] == "ROLE_ADMIN" )
                    {
                        ...
                        for ($i=0; $i <count($affectCompte) ; $i++) { 
                            ...
                            if ($userAffect == $user->getUserComptePartenaire() && ( ($dateDebut <= $data->getDateDebut() && $dateFin >= $data->getDateDebut()) || ($dateDebut <= $data->getDateFin() && $dateFin >= $data->getDateFin()) || ($dateDebut >= $data->getDateDebut() && $dateFin <= $data->getDateFin())))
                            {
