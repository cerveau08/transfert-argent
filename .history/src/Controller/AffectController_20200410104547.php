                ...
                if($user->getProfil()->getLibelle() === 'ROLE_CAISSIER_PARTENAIRE' && $nineauser === $user->getPartenaire()->getNinea())
                    ...
                    //  dd($affects);
                        foreach($affects as $affect) {
                          // dd($affect->getUserComptePartenaire());
                           if(($affect->getDateDebut() >= $values->debut && $affect->getDateDebut() >= $values->fin) || ($affect->getDateFin() <= $values->debut && $values->getDateFin() <= $values->fin)) {
                             $data[$i]=$user;
                             $i++;
                           }
                        }
