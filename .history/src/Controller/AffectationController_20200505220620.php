...
//Récupération date début date fin
            ...
            //comparaison pour savoir le la date courante est compris entre la date de début et la detae de fin
                ...
                if ($dateDebut <= $dateCourent && $dateCourent <= $dateFin) 
                {
                    ...
                    if ($data->getUser()->getRoles()[0] == "ROLE_ADMIN_SYSTEME" ||
                        $data->getUser()->getRoles()[0] == "ROLE_CAISSIER" ||
                        $data->getUser()->getRoles()[0] == "ROLE_ADMIN" )
                    {
                        ...
                        }elseif ($data->getUser()->getPartenaire()->getNinea() != $data->getUserAfect()->getPartenaire()->getNinea()) {
                        ...
                        //Controler si userPartenaire courant est affecte a un compte compte dans cette mm periode 
                          ...
                          for ($i=0; $i <count($affectCompte) ; $i++) { 
                              ...
                              $userAffect = $affectCompte[$i]->getUser();
                            
                             if ($userAffect == $data->getUser() &&( $dateDebut == $data->getDateDebut()) ||  $dateFin == $data->getDateFin() )
                              {
