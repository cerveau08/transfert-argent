           ...
           // dd($values);
            ...
            foreach($users as $user)
                ...
                if($user->getProfil()->getLibelle() === 'ROLE_CAISSIER_PARTENAIRE' && $nineauser === $user->getPartenaire()->getNinea())
                    {
                    //  dd($affects);
                   // $affectCompte = $user->$this->affectCompteRepo->findAll();
                    //dd($affectCompte);
                   ...
                   // dd($affectCompte);    
                    ...
                    for ($i=0; $i <count($affectCompte) ; $i++) { 
                        ...
                        if($userAffect == $user->getaffectationsaffectations() && (($dateDebut >= $values->debut && $dateDebut >= $values->fin) || ($dateFin <= $values->debut && $dateFin <= $values->fin))) {
                          $data[$j]=$user;
                          $j++;
                        }else{
                      $data = [
